...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.08 * COIN;	// 8% Interest 
static const int64_t COIN_YEAR_REWARD = 8 * CENT; //(output to console will be updated)
...
public:
    ...
    {
        // samurai: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // samurai: get transaction coin age
...
public:
    ...
    // samurai: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // samurai: two types of block: proof-of-work or proof-of-stake
    ...
    // samurai: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // samurai: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // samurai: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // samurai: block index flags
    enum  
    {
